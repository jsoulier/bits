...
def get_lookup(filename, key, name, id):
    lookup = {}
    ...
    for line in text.splitlines():
        if not line or line.startswith("#"):
            continue
        fields = line.split("\t")
        lookup[fields[key]] = (fields[name], int(fields[id]))
    return lookup
...
def get_display_names(ids, language):
    ...
    with archive.open("alternateNames.txt") as raw:
        for line in io.TextIOWrapper(raw, encoding="utf-8"):
            ...
            current = display.get(id)
            if current is None or (preferred and not current[1]):
                display[id] = (name, preferred)
    return {id: name for id, (name, _) in display.items()}
...
def main():
    ...
    for row in rows:
        for admin in (row[2], row[3], row[4]):
            if admin and admin[1]:
                ids.add(admin[1])
    display = get_display_names(ids, args.lang)
    ...
    def insert(row):
        id, name, county, state, nation, latitude, longitude, population = row
        parts = [display.get(id) or name]
        for admin in (county, state, nation):
            if admin:
                parts.append(display.get(admin[1]) or admin[0])
        location = ", ".join(parts)
        return (location, latitude, longitude, population)
